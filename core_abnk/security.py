...
import requests
import logging
from core_abnk import settings as myinfo_settings
...
def generate_client_assertion(url: str, jkt_thumbprint: str) -> str:
    ...
    """Generate client assertion with correct Singapore time"""

    from datetime import datetime, timedelta

    # Convert local Jakarta time to Singapore time (UTC+8)
    # now_utc = datetime.utcnow()  # UTC time
    # now_sgt = now_utc + timedelta(hours=1)  # Convert to SGT
    #
    # now = int(now_sgt.timestamp())
    now = int(time.time())
    payload = {
